...
class Router(object):
	def __init__(self, net):
		...
		# ip address(str): MAC address (MAC)
		arp_table = {}

		# a list of list storing [IPv4Network (prefix/mask), next hop ip (str), interface name (str), current time]
		dynamic_routing_table = []
		DYNAMIX_ROUTING_TABLE_SIZE = 5

		forwarding_table = []
		# read forwarding_table.txt into forwarding_table 
		# [IPv4Network(prefix/mask), next hop ip (str), port name (str)]
			...
			if gotpkt:
				...
				# for IPv4
					...
					if ip.dst not in [intf.ipaddr for intf in interfaces]:
						...
						for entry in self.dynamic_routing_table:
							if ip.dst in entry[0]: # in means ip address matches prefix/mask
						...
						if match_entry != None: 
							...
							print(self.arp_table)
							print(match_entry[1])
							# if next hop ip address is none, change to dst ip addr, a string
							...
							if str(next_hop_IP) in self.arp_table: 
								...
								print(match_entry)
								print(self.arp_table[str(next_hop_IP)])
								...
								e.src = self.net.interface_by_name(match_entry[2]).ethaddr
							...
							# send an ARP query in order to obtain Ethernet address corresponding to next hop IP addr
								...
								# if there is already an unreplied arp request for this next hop ip, which means ip in ip_packet_queue
				...
				# for dynamic routing message
					...
					if len(self.dynamic_routing_table) >= self.DYNAMIX_ROUTING_TABLE_SIZE:	
						...
						self.dynamic_routing_table.pop() # remove oldest entry
					# add new one tolist 
					self.dynamic_routing_table.append([IPv4Network(str(pkt.get_header(DynamicRoutingMessage)._advertised_prefix)
							...
							str(pkt.get_header(DynamicRoutingMessage)._next_hop), input_port, time.time()])
					# sort by prefix length
					self.dynamic_routing_table.sort(key = lambda x: x[0].prefixlen, reverse = True)
					print(self.dynamic_routing_table)
