...
def router_tests():
	...
	# 10 test dynamic table get updated, received packet
	packet = mk_pkt(hwsrc = '10:00:00:00:00:03', hwdst =  '30:00:00:00:00:01', ipsrc  = '192.168.1.100', ipdst = '172.0.128.0')
   s.expect(PacketInputEvent("router-eth0", packet), "IP packet to be forwarded to 172.0.128.0 should arrive on router-eth0")
	
	# 11 use dynamic table for match, next hop ip is 192.168.1.2
	arp_request  = create_ip_arp_request('10:00:00:00:00:01', '192.168.1.1', '192.168.1.2')
	s.expect(PacketOutputEvent("router-eth0", arp_request), "Router should send ARP request for out router-eth0 interface")

	# 12 get arp reply
	arp_response = create_ip_arp_reply('60:00:00:00:00:01', '10:00:00:00:00:01',
   ...
   s.expect(PacketInputEvent("router-eth0", arp_response), "Router should receive ARP response for 192.168.1.2 on router-eth0 interface")
	...
	# 13 send ip packet 
	packet = mk_pkt(hwsrc='10:00:00:00:00:01', hwdst='60:00:00:00:00:01', ipsrc='192.168.1.100', ipdst='172.0.128.0', ttl=63)
   s.expect(PacketOutputEvent("router-eth0", packet), "IP packet should be forwarded to 172.0.128.0 out router-eth0")
