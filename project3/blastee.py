...
def switchy_main(net):
	...
	while True:
		...
		if gotpkt:
			...
			print(pkt[3])
			print(pkt[3].data)
			print(type(pkt[3].data))
			print(int.from_bytes(pkt[3].data[:4], 'big'))
			print(pkt[3].data[6:8].decode())
			'''
			...
			sequence_num = int.from_bytes(contents.data[:4], 'big')
			payload = contents.data[6:]
			'''
			new_pkt += seq_num.to_bytes(4, 'big')
			print(int.from_bytes(pkt[3].data[4:6], 'big'))
